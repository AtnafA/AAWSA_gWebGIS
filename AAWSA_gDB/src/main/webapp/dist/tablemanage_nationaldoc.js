                ...
                function acessvec_detail(datafrom){
                	...
                	for(var i=0; i< datafrom.length; i++){
                		 if (datafrom[i].spatial_or_non === "Corporation Project"&& datafrom[i].file_gr === "Time Series Data"){
                			datadrop+='<li style="margin-left: 3.3em;">Thematic Area:&nbsp; '+datafrom[i].datacl+'</li>'
                  			...
                  			+'<li style="margin-left: 5.1em;">Project type:&nbsp; '+datafrom[i].basename+'</li>'
	...
	||datafrom[i].file_ext === "{csv}" || datafrom[i].file_ext === "{docx}" || datafrom[i].file_ext === "{pdf}" || datafrom[i].file_ext === "{png}" 
		||datafrom[i].file_ext === "{tif}" || datafrom[i].file_ext==="{tiff}"|| datafrom[i].file_ext==="{tff}"||datafrom[i].file_ext === "{xls}"
                            	  || datafrom[i].file_ext === "{jpg}" || datafrom[i].file_ext === "{gif}" || datafrom[i].file_ext === "{dwg}" ||datafrom[i].file_ext==="{zip}"
		             ||datafrom[i].file_ext === "{las}" || datafrom[i].file_ext==="{LAS}"|| datafrom[i].file_ext==="{laz}" || datafrom[i].file_ext==="{LAZ}"
		             ||datafrom[i].file_ext === "{ZIP}" || datafrom[i].file_ext==="{rar}"|| datafrom[i].file_ext==="{RAR}" || datafrom[i].file_ext==="{tar}"|| datafrom[i].file_ext==="{GZ}"){
               			datadrop+='<li style="margin-left: 0.4em;">Thematic Area:&nbsp; '+datafrom[i].datacl+'</li>'
                 			+'<li style="margin-left: 0.9em;">Classification:&nbsp; '+datafrom[i].folder+'</li>'
                 			+'<li style="margin-left: 1.4em;">Project type:&nbsp; '+datafrom[i].basename+'</li>'
                 			+'<li style="margin-left: 2.8em;">File Type:&nbsp; '+datafrom[i].file_gr+'</li>'
                 			+'<li style="margin-left: 3.5em;">Format:&nbsp; '+datafrom[i].file_ext+'</li>'
                 			+'<li style="margin-left: 4.3em;">Yeare:&nbsp; '+datafrom[i].date+'</li>'
                 			+'<li style="margin-left: 6.0em; margin-top: 0.1em;"><a style="color: blue;" href="download?file_id=' 
               			+ datafrom[i].name+'&pro_name='+datafrom[i].cata_gory+'&pro_id='+datafrom[i].project_id+'&typ_name='+datafrom[i].datacl
               			+'&cat_name='+datafrom[i].folder+'">Download</a></li>';	
               		}
                	}
